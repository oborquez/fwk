...
class CrudCode extends CCodeModel
{
	...
	{
		if($column->type==='boolean')
			return "\$form->checkBox(\$model,'{$column->name}',array('class'=>'".$htmlOptions."'))";
		elseif(stripos($column->dbType,'text')!==false)
			return "\$form->textArea(\$model,'{$column->name}',array('rows'=>6, 'cols'=>50, 'class'=>'".$htmlOptions."'))";
		else
		{
			if(preg_match('/^(password|pass|passwd|passcode)$/i',$column->name))
			...
			else
			{
				if(($size=$maxLength=$column->size)>60)
					$size=60;
				return "\$form->{$inputField}(\$model,'{$column->name}',array('size'=>$size,'maxlength'=>$maxLength, 'class'=>'".$htmlOptions."'))";
			}
		}
	}
